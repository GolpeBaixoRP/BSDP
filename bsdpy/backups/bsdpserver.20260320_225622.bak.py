#!/usr/bin/env python2
req_ip_opt = packet.GetOption('request_ip_address')
try:
    if req_ip_opt and len(req_ip_opt) == 4:
        clientip = ipv4(req_ip_opt)
    else:
        clientip = ipv4('0.0.0.0')
except Exception:
    import logging
    logging.exception('Invalid request_ip_address from client, falling back to 0.0.0.0')
    clientip = ipv4('0.0.0.0')
try:
if req_ip_opt and len(req_ip_opt) == 4:
clientip = ipv4(req_ip_opt)
...
try:
forced_ip = os.environ.get('DOCKER_BSDPY_IP') or os.environ.get('BSDPY_IP')
if forced_ip:
externalip = forced_ip
serverhostname = externalip
serverip = map(int, externalip.split('.'))
serverip_str = externalip
myip = externalip
logging.debug('Using forced external IP %s' % externalip)
...
if not myip:
raise Exception('Could not determine IP for interface %s; set BSDPY_IP explicitly' % serverinterface)
serverhostname = myip
serverip = map(int, myip.split('.'))
serverip_str = myip
logging.debug('No BSDPY_IP env var found, using IP from %s interface' % serverinterface)
