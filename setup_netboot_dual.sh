...
# ESTE caminho precisa ser um root filesystem COMPLETO do macOS para NFS boot
...
echo
echo "==> 6) Escrevendo BSDP server dual-mode"
python3 - <<PY
from pathlib import Path

bsdp = f'''#!/usr/bin/env python2
...
'''
images = []
if int("${MAINT_ENABLED}") == 1:
    images.append(f'''    {{
        "id": {MAINT_ID},
        "name": "{MAINT_NAME}",
        "boot_file": "{MAINT_BOOT_FILE}",
        "root_path": "nfs:{SERVER_IP}:{MAINT_NFS_ROOT}",
        "mode": "nfs"
    }}''')
if int("${INSTALL_ENABLED}") == 1:
    images.append(f'''    {{
        "id": {INSTALL_ID},
        "name": "{INSTALL_NAME}",
        "boot_file": "{INSTALL_BOOT_FILE}",
        "root_path": "http://{SERVER_IP}/NetBoot/NetBootSP0/install.nbi/NetInstall.dmg",
        "mode": "http"
    }}''')
...
def parse_selected_image_id(veo):
    # espera tag 8 len 4 com bytes do image_id na forma 81 00 XX YY
    i = 0
    while i + 1 < len(veo):
        tag = veo[i]
        ln = veo[i+1]
...
def build_bsdp_list_payload():
    # tag 1 = message type LIST
    # tag 4 = server priority
    # tag 7 = default image
    # tag 9 = image list
...
Path("/opt/bsdpy/bsdpserver.py").write_text(bsdp)
PY
...
[Service]
Type=simple
ExecStart=${PYTHON:-${PYTHONPATH:-/usr/local/bin/python2.7}} ${BSDP_FILE}
Restart=always
RestartSec=2
...
# Corrige o caminho real do Python 2.7 se estiver em outro lugar
...
sed -i "s|/usr/local/bin/python2.7|${PY27}|g" "${BSDP_SERVICE}"
